...
def conv1x1(in_planes, out_planes, stride=1, Nbits=4, bin=True,mask_initial_value=0):
    "1x1 convolution with padding"
    return BitConv2d(in_planes, out_planes, kernel_size=1, stride=stride,
                     padding=0, bias=False, Nbits = Nbits, bin=bin,mask_initial_value=mask_initial_value)
...
class Bottleneck(nn.Module):
    # expansion = 4
    def __init__(self, inplanes, mid_planes, planes, stride=1, downsample=False, Nbits=4, act_bit=4, bin=True,mask_initial_value=0):
        super(Bottleneck, self).__init__()
        self.conv1 = conv1x1(inplanes, mid_planes, stride=1, Nbits=Nbits, bin=bin,mask_initial_value=mask_initial_value)
        self.bn1 = nn.BatchNorm2d(mid_planes)
        self.conv2 = conv3x3(mid_planes, mid_planes, stride, Nbits=Nbits, bin=bin,mask_initial_value=mask_initial_value)
        self.bn2 = nn.BatchNorm2d(mid_planes)
        self.conv3 = conv1x1(mid_planes, planes, stride=1, Nbits=Nbits, bin=bin,mask_initial_value=mask_initial_value)
...
class ResNet50(MaskedNet):
    def __init__(self, num_classes=1000, Nbits=6, act_bit=0, bin=True, mask_initial_value=0.):
        super(ResNet50, self).__init__()

        self.conv1 = BitConv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False, Nbits=8, bin=bin, mask_initial_value=mask_initial_value)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.layer1_0 = Bottleneck(64, 64, 256, stride=2, downsample=True, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer1_1 = Bottleneck(256, 64, 256, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer1_2 = Bottleneck(256, 64, 256, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer2_0 = Bottleneck(256, 128, 512, stride=2, downsample=True, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer2_1 = Bottleneck(512, 128, 512, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer2_2 = Bottleneck(512, 128, 512, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer2_3 = Bottleneck(512, 128, 512, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer3_0 = Bottleneck(512, 256, 1024, stride=2, downsample=True, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer3_1 = Bottleneck(1024, 256, 1024, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer3_2 = Bottleneck(1024, 256, 1024, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer3_3 = Bottleneck(1024, 256, 1024, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer3_4 = Bottleneck(1024, 256, 1024, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer3_5 = Bottleneck(1024, 256, 1024, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer4_0 = Bottleneck(1024, 512, 2048, stride=2, downsample=True, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.layer4_1 = Bottleneck(2048, 512, 2048, stride=1, downsample=False, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        ...
        self.fc = BitLinear(2048, out_features=num_classes, Nbits=Nbits, bin=bin,mask_initial_value=mask_initial_value)
    ...
    def forward(self, x):
        ...
        x = self.layer1_0(x, self.temp, self.ticket)
        x = self.layer1_1(x, self.temp, self.ticket)
        x = self.layer1_2(x, self.temp, self.ticket)
        x = self.layer2_0(x, self.temp, self.ticket)
        x = self.layer2_1(x, self.temp, self.ticket)
        x = self.layer2_2(x, self.temp, self.ticket)
        x = self.layer2_3(x, self.temp, self.ticket)
        x = self.layer3_0(x, self.temp, self.ticket)
        x = self.layer3_1(x, self.temp, self.ticket)
        x = self.layer3_2(x, self.temp, self.ticket)
        x = self.layer3_3(x, self.temp, self.ticket)
        x = self.layer3_4(x, self.temp, self.ticket)
        x = self.layer3_5(x, self.temp, self.ticket)
        x = self.layer4_0(x, self.temp, self.ticket)
        x = self.layer4_1(x, self.temp, self.ticket)
        x = self.layer4_2(x, self.temp, self.ticket)
