...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, Nbits=6, act_bit=0, bin=True, mask_initial_value=0.):
...
class VGG19bn(nn.Module):
    ...
    def __init__(self, num_classes=10, Nbits=8, act_bit=0, bin=True, mask_initial_value=0.):
        ...
        self.block1 = BasicBlock(3, 64, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)

        self.block2 = BasicBlock(64, 128, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)

        self.block3_1 = BasicBlock(128, 256, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.block3_2 = BasicBlock(256, 256, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)

        self.block4_1 = BasicBlock(256, 512, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
        self.block4_2 = BasicBlock(512, 512, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)

        self.block5_1 = BasicBlock(512, 512, Nbits=Nbits, act_bit=act_bit, bin=bin, mask_initial_value=mask_initial_value)
    ...
    def forward(self, x):
        x = self.block1(x, self.temp, self.ticket)
        x = self.maxpool(x)

        x = self.block2(x, self.temp, self.ticket)
        x = self.maxpool(x)

        x = self.block3_1(x, self.temp, self.ticket)
        x = self.block3_2(x, self.temp, self.ticket)
        x = self.maxpool(x)

        x = self.block4_1(x, self.temp, self.ticket)
        x = self.block4_2(x, self.temp, self.ticket)
        x = self.maxpool(x)

        x = self.block5_1(x, self.temp, self.ticket)
        x = self.block5_2(x, self.temp, self.ticket)
        x = self.maxpool(x)
